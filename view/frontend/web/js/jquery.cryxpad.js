  ...
  $.fn.cryxpad = function (options) {
    ...
    return this.each(function () {
      ...
      element.css({
        ...
        "margin-right": "auto",
        width:
          parametres.width * parametres.carreaux +
          parametres.carreaux * parametres.button.marginLeft +
          3,
        height:
          parametres.width * parametres.linhas +
          parametres.linhas * parametres.button.marginLeft +
          3,
      ...
      for (i = 0; i < parametres.carreaux; i++) {
        ...
        for (j = 0; j < parametres.linhas; j++) {
          a[i][j] = "[" + i + "," + j + "]";
          var tt = $("<button type='button' class='code-btn'></button>")
            .css({
              "font-size": "x-large",
              background: "#143080",
              position: "absolute",
      ...
      for (var i = 1; i < 10; i++) {
        ...
        if (tt == 0) {
          ...
          var y = getRandomInt(0, parametres.linhas - 1);
          var data = {
            x: x,
            y: y,
          };
      ...
      $(document).on(
        "click",
        "." + parametres.containerClass + " .code-btn",
        function (e) {
          e.preventDefault();
          e.stopPropagation();
          buttonClik($(this));
    ...
    function removeButtonClick($el) {
      var inputCode = $inputcodeElement.val(function (index, value) {
